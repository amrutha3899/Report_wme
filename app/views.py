...
def upd_config():
    ...
    config_path = 'C:/inetpub/wwwroot/FlaskApp/config.py'
    with open(config_path, 'w') as config_file:
        config_file.write(f"server = '{server}'\n")
        config_file.write(f"database = '{database}'\n")
        config_file.write(f"table_name = 'sampletable'\n")
...
def status_report_steps():
    ...
    description = "Please follow the below steps to generate the status report:<br>" \
                  "<br>"\
                  "1. Enter the name of the database you want to generate a report for. <br>" \
...
def change():
    ...
    formatted_time = datetime.now().strftime('%H:%M:%S_%Y-%m-%d')
